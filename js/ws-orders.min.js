...
(function () {
  ...
  function startSSE() {
    ...
    source.onerror = function () {
      ...
      if (!opened) {
        ...
        sseErrorCount += 1;
        // After a few protocol errors/disconnects, polling is more stable on some clients/proxies.
        if (sseErrorCount >= 3) {
          startPolling();
          return;
        }
  ...
  function stopLiveUpdates() {
    // Ensure staff pages don't keep SSE/polling running while navigating away or backgrounded.
    // This also reduces the chance of exhausting php-fpm workers with long-lived SSE requests.
    if (reconnectTimer) {
      clearTimeout(reconnectTimer);
      reconnectTimer = null;
    }
  ...
  // Stop live channels when leaving the page and when app is backgrounded.
  ...
  document.addEventListener("visibilitychange", function () {
    ...
    // Restart after a tiny delay to avoid immediate reconnect loops on some mobile browsers.
    setTimeout(function () {
      if (!pollingActive) {
        startSSE();
      }
