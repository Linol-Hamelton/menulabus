...
| inventory MVP | ingredient stock + recipes + append-only `stock_movements` audit log, suppliers contact book, auto-deduction hook on `order.created` (aggregates duplicate dishes into one UPDATE), throttled low-stock alerts (Telegram + `inventory.stock_low` webhook), `/admin/inventory.php` with inline editing + `±` adjust + history drawer, per-menu-item recipe editor embedded in `/admin/menu.php`, **Phase 33** — structured unit dropdown (`UnitCatalog` of 7 canonical units + «Другое» free-text fallback) and CSV import of recipes (per-dish merge/replace + optional auto-create of ingredients) launched from the recipe modal, **Phase 34** — `/admin/inventory.php` UX overhaul (admin-pane-header / kicker / caption pattern matching `/admin/menu.php`, 3 summary-cards for active count/low-stock/total stock value, view-toggle Active/Archived via `?view=`, filter-bar by name/supplier/stock-status, bulk-archive with checkbox column), `db.php::getRecipeCost()` + `getInventoryValueSummary()` + live «Себестоимость рецепта» in recipe modal, `getDishMargins()` extended with `ingredient_cogs_per_unit` / `ingredient_margin*` columns surfaced as «Ингр. cost» column in owner analytics-v2 with «Точная»/«Грубая» accuracy badge, lifecycle webhooks `ingredient.archived` / `ingredient.restored` / `ingredient.cost_changed` (no-op compare) plus admin/webhooks event-picker now exposes `inventory.stock_low` + the three new ingredient events | `sql/inventory-migration.sql`, `sql/recipes-import-migration.sql`, `lib/Inventory/UnitCatalog.php`, `db.php::listIngredients(includeArchived,search,supplierId,stockStatus)`, `admin/inventory.php`, `admin/menu.php`, `admin/webhooks.php`, `api/save-inventory.php` (`bulk_archive_ingredients`), `api/import-recipes.php`, `download-recipe-sample.php`, `js/admin-inventory.js`, `js/admin-recipe.js`, `js/owner-analytics-v2.js`, `css/admin-inventory.css`, `css/admin-recipe.css`, `css/admin-design-modals.css`, `css/owner-analytics-v2.css`, `lib/WebhookDispatcher.php`, hook points in `create_new_order.php` / `api/v1/orders/create.php` / `api/save-group-order.php`, `tests/InventoryTest.php` | `inventory` | implemented |
...
| Route / feature | Intended behavior | Planned files | Doc target | Roadmap status |
|---|---|---|---|---|
| SaaS billing engine v3 (Phase 32) | Pro Trial 90d / Pro 6 990 / Pro-annual 69 900 / Enterprise 19 990 (with bundled onboarding+training) / Enterprise-annual 199 900 / Enterprise+ договорная — sales-led. Killed Starter tier. YooKassa recurring, soft dunning (read-only at d8, suspend at d30), self-service signup, provider admin | `lib/Billing/{PlanRegistry,FeatureGate,SubscriptionStore,YookassaRecurring}.php`, `sql/billing-migration.sql` (control plane), `signup.php` + `api/signup.php`, `owner.php?tab=billing`, `provider/{billing,tenant}.php`, `scripts/billing-cycle-worker.php` (cron `0 */6 * * *`), `index.php` provider-mode pricing section | [`billing.md`](billing.md), [`marketing-strategy-2026.md`](marketing-strategy-2026.md) | **Implemented (Phase 32A)** — pricing live; card-required signup + addon purchase API + annual billing automation + trial-end conversion cron in Phase 32B |
| Add-on services catalog | Express onboarding 9 900 / Full 24 900 / Migration 49 900 / Group training 6 900 / Individual training 4 900 / Telegram-bot setup 4 900 / Custom domain 2 900 / Recorded video 2 900 / Priority support +990/mo | `lib/Billing/AddonCatalog.php` (new in 32B), `addon_purchases` table (new in 32B), `/api/billing-purchase-addon.php` (new in 32B), display surface on `/owner.php?tab=billing` and `/index.php#pricing` (already in 32A) | [`marketing-strategy-2026.md`](marketing-strategy-2026.md) | **Display-only (32A)** — purchase API + persistence in 32B |
...
| compliance pack — 2FA + audit log | TOTP enrolment for admin/owner, immutable audit log of state changes | `lib/AuditLog.php`, `audit_log` table, `account.php?tab=security` 2FA wizard | `security-hardening-roadmap.md` Phase 12 | **Implemented** |
| compliance pack — GDPR/152-ФЗ/ЕГАИС/Меркурий | Data export / right to delete (GDPR + 152-ФЗ), alcohol regulator (ЕГАИС), meat/fish regulator (Меркурий) | `lib/compliance/*` (new), `admin-security.php` extensions | `compliance.md` (new) | planned |
...
| onboarding (current MVP) | 5-step wizard: project name → logo upload → primary color picker → first QR code → done. Wired to `/api/signup.php` flow. | `onboarding.php` (Phase 4), `css/onboarding.css` | already shipped — see Phase 4 | **Implemented** |
| onboarding 2.0 | Template library (fast-food / fine-dining / cafe / bar / pizzeria), interactive demo, in-app help center, chat support — distinct upgrade over the current 5-step wizard | `scripts/tenant/templates/*`, `onboarding.php` overhaul, `help.php` knowledge-base | `onboarding-2.md` (new) | planned |
