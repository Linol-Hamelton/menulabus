...
| inventory MVP | ingredient stock + recipes + append-only `stock_movements` audit log, suppliers contact book, auto-deduction hook on `order.created` (aggregates duplicate dishes into one UPDATE), throttled low-stock alerts (Telegram + `inventory.stock_low` webhook), `/admin-inventory.php` with inline editing + `±` adjust + history drawer, per-menu-item recipe editor embedded in `/admin-menu.php`, **Phase 33** — structured unit dropdown (`UnitCatalog` of 7 canonical units + «Другое» free-text fallback) on `/admin/inventory.php` and CSV import of recipes (per-dish merge/replace + optional auto-create of ingredients) launched from the recipe modal in `/admin/menu.php` | `sql/inventory-migration.sql`, `sql/recipes-import-migration.sql`, `lib/Inventory/UnitCatalog.php`, `db.php::`, `admin/inventory.php`, `admin/menu.php`, `api/save-inventory.php`, `api/import-recipes.php`, `download-recipe-sample.php`, `js/admin-inventory.js`, `js/admin-recipe.js`, `css/admin-inventory.css`, `css/admin-recipe.css`, `css/admin-design-modals.css`, recipe section in `admin-menu.php`, hook points in `create_new_order.php` / `api/v1/orders/create.php`, `tests/InventoryTest.php` | `inventory` | implemented |
