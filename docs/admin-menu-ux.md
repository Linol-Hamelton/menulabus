...
### Phase 17.2 — fix Brand modal form layout (v2.2.2, 2026-05-05)

После Phase 17 поля Brand modal (Название/Слоган/Описание/URL/favicon/...) рендерились inline без legacy `.brand-fields` обёртки. Добавлены explicit CSS правила для `.design-modal#brandModal .admin-label/.admin-input/.brand-logo-preview/.launch-readiness-card` и для `.design-modal#fontsModal .font-control` — все поля теперь vertical stack с правильными отступами.
